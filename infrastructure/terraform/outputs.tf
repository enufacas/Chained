...
output "ai_native_control_plane_url" {
  ...
  value       = try(google_cloud_run_v2_service.ai_control_plane.uri, "Not deployed")
}

output "ai_native_infra_runner_url" {
  description = "URL of the Infra Runner service"
  value       = try(google_cloud_run_v2_service.infra_runner.uri, "Not deployed")
}

output "ai_native_database_instance" {
  description = "Cloud SQL instance name for AI-Native Control Plane"
  value       = try(google_sql_database_instance.ai_native_db.name, "Not deployed")
}

output "ai_native_database_connection" {
  description = "Cloud SQL connection name"
  value       = try(google_sql_database_instance.ai_native_db.connection_name, "Not deployed")
}

output "ai_native_database_private_ip" {
  description = "Private IP address of Cloud SQL instance"
  value       = try(google_sql_database_instance.ai_native_db.private_ip_address, "Not deployed")
  sensitive   = true
}

output "ai_native_vpc_connector" {
  description = "VPC Access Connector for Cloud Run"
  value       = try(google_vpc_access_connector.ai_native_connector.name, "Not deployed")
}
...
output "ai_native_deployment_summary" {
  ...
  value = <<-EOT
    ...
    ╠════════════════════════════════════════════════════════════════════════════╣
    ║                                                                             ║
    ║  🧠 AI Control Plane:   ${try(google_cloud_run_v2_service.ai_control_plane.uri, "Not deployed")}
    ║                                                                             ║
    ║  🏗️  Infra Runner:       ${try(google_cloud_run_v2_service.infra_runner.uri, "Not deployed")}
    ║                                                                             ║
    ║  🗄️  Database:           ${try(google_sql_database_instance.ai_native_db.name, "Not deployed")}
    ...
    ║                                                                             ║
    ║  🔧 VPC Connector:      ${try(google_vpc_access_connector.ai_native_connector.name, "Not deployed")}
    ║                                                                             ║
    ╚════════════════════════════════════════════════════════════════════════════╝
    ...
    2. Run database migrations:
       kubectl run psql --image=postgres:15 --rm -it -- psql "${try(format("postgresql://ai_native_admin@%s/ai_native_control_plane", google_sql_database_instance.ai_native_db.private_ip_address), "")}"
       \i /migrations/001_initial_schema.sql
       \i /migrations/002_add_vector_support.sql
    
    3. Test the API:
       curl -X POST ${try(google_cloud_run_v2_service.ai_control_plane.uri, "")}/execute \
