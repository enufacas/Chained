...
the tech-lead-changes-requested label by checking if tech lead review was
actually required before blocking the merge.

Addresses the issue where PR #2417 was blocked even though no tech lead
review was needed (no tech lead tags present).
"""
...
def test_workflow_has_tech_lead_validation():
    ...
    before blocking on tech-lead-changes-requested label.
    """
    ...
    if 'Only block for tech-lead-changes-requested if tech lead review was actually required' in content:
        print(f"  ✓ Contains comment explaining the validation logic")
    else:
...
def test_merge_eligibility_scenarios():
    ...
    This simulates the bash logic used in the workflow.
    """
    print(f"\n✅ Testing merge eligibility scenarios...")
    
    scenarios = [
        {
            'name': 'Stale label - no tech lead required',
            'has_changes_requested': True,
            'requires_tech_lead': False,
            'tech_leads': '',
            'expected_blocked': False,
            'reason': 'Label ignored when tech lead not required'
        },
        {
            'name': 'Valid label - tech lead required',
            'has_changes_requested': True,
        ...
        {
            'name': 'No changes requested label',
            'has_changes_requested': False,
            'requires_tech_lead': True,
            'tech_leads': 'workflows-tech-lead',
            'expected_blocked': False,
            'reason': 'Allow when no changes requested'
        },
        {
            'name': 'Multiple tech leads assigned',
            'has_changes_requested': True,
            'requires_tech_lead': False,
    ...
    for scenario in scenarios:
        ...
        if scenario['has_changes_requested']:
        ...
        print(f"     has_changes_requested={scenario['has_changes_requested']}, "
...
def main():
    ...
    print("the workflow blocked PRs with tech-lead-changes-requested label")
    print("even when tech lead review was never required.")
