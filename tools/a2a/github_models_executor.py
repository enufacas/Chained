...
The GitHub Models API is accessed via:
  ...
  - Authentication: `Authorization: Bearer YOUR_PAT` (standard Bearer auth)
...
Common Errors:
  - "No access to model": COPILOT_PAT is missing `models:read` scope
...
class GitHubModelsAgent:
    ...
    Authentication: Uses `Authorization: Bearer` format (standard OAuth2).
...
{f"Additional Context: {context}" if context else ""}"""
    ...
    async def _call_github_models_api_with_tools(
        ...
        # Note: GitHub Models API now prefers Bearer authentication (updated Dec 2024)
    ...
    async def _call_github_models_api(self, messages: list) -> str:
        ...
        Uses standard Bearer authentication with a fine-grained PAT
        that has `models:read` scope.
        """
